...
var qr_text = prompt("Enter text", "text qr");
var layer_name = "qr_959";
...
var qrSize = 144;
	...
	if ( qrData !== '' ) {
		...
		// рисуем подложку под котом. Не ебу, как оно работает - подложку подгонял.
       // var qrSize = 144;
    	// qrSize = parseInt(win.sizeGroup.qrSize.text) * 72
		var hh = 4;
		//var  x1 =  0, y1 = 0;
		//var rectbkg =  app.activeDocument.pathItems.rectangle( x1+qrSize/2+hh, y1-hh, (qrSize+hh*2), (qrSize +hh*2));
		var rectbkg =  app.activeDocument.pathItems.rectangle( 845.89, -4,(qrSize+hh*2), (qrSize +hh*2));
		
		qr.addData( qrData);
		qr.make();

		IllQR.init( qr.modules, app.activeDocument );
...
var windowRes = "dialog { text:'Illustrator QR', \
					 sizeGroup: Group {\
						label1: StaticText { text:'Не менять. Жать ОК. QR Size (in inches)',\
